...
{
  this._application.map_object()._map.on('zoomend moveend', function(e){
...
Grid.prototype.get_completeness_from_bbox = function(bbox)
{
  ...
  /* SW Lon, SW Lat, NE Lon, NE Lat
     adjust bbox to grid
  */
  // bbox = this.adjust_bbox2grid(bbox, cell_side);
...
Grid.prototype._cell_coords_by_line = function(bbox, cell_side)
{
  /* SW Lon, SW Lat, NE Lon, NE Lat */
  ...
  do {
    // let cc_line = [];
    let i = 0;
    // console.log(lon, '<', lon_limit);
    for (let lat=bbox[3]; lat >=bbox[1]; lat-=cell_side) {
  ...
  // console.log('... with cell size', cell_side, ', coords2id new test', this._coords2id_new([[23.8 , 37.90555556], [23.80277778, 37.90555556], [23.80277778, 37.90277778], [23.8       , 37.90277778], [23.8, 37.90555556]], 360.0),
  //             this._coords2id_new([[23.8       , 37.90277778],
  //                                  [23.80277778, 37.90277778],
  //                                  [23.80277778, 37.9       ],
  //                                  [23.8       , 37.9       ],
  //                                  [23.8       , 37.90277778]], 360.0),
  //             this._coords2id_new([[23.8       , 37.9       ],
  //                                  [23.80277778, 37.9       ],
  //                                  [23.80277778, 37.89722222],
  //                                  [23.8       , 37.89722222],
  //                                  [23.8       , 37.9       ]], 360.0),
  //             this._coords2id_new([[23.8       , 37.89722222],
  //                                  [23.80277778, 37.89722222],
  //                                  [23.80277778, 37.89444444],
  //                                  [23.8       , 37.89444444],
  //                                  [23.8       , 37.89722222]], 360.0));
...
Grid.prototype._ids_coords_by_line = function(cell_coords_lines)
{
  ...
  for (let i=0; i<ids_coords_lines.length;i++) {
    ...
    $.each(cell_coords_lines[i], function(_, coords) {
      ...
      let id = this._coords2id(coords);

      let id_new = this._coords2id_new(coords, this._cell_frac_denominator);
      if (id !== id_new) {
        console.error('id', id, 'does not match id (new)', id_new);
        got_id_error = true;
      } else {
        if (got_id_error === true) {
          console.error('id', id, 'does match id (new)', id_new);
        }
      }
...
/** calculates  */
Grid.prototype.features_grid_from_bbox = function(bbox)
{
  console.log('features grid from bbox');
  let cell_side = this._cell_width; // the side of a cell in both lon lat direction
  /* the corner coordinates of all cells by line */
  let cell_coords_lines = this._cell_coords_by_line(bbox, cell_side);
  ...
  let features = [];
  this.binaries_lines = new Array(cell_coords_lines.length);
  let line_cnt = 0;
...
*/
Grid.prototype.draw_grid = function(e)
{
  ...
  if (this._max_zoom <= zoom) {
    let bbox = this._map_object.map().getBounds().toBBoxString().split(',').map(Number);
    // southwest_lng, southwest_lat, northeast_lng, northeast_lat

    // the squares grid 'comes' as a promise object, so we have to get it with 'then'
    this.features_grid_from_bbox(bbox).then(function(result) {
